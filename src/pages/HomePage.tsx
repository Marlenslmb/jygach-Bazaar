    ...
    <div className="animate-page-in">
      ...
      <section className="grid lg:grid-cols-[1.2fr_1fr] gap-10 mb-16 items-end">
        <div className="py-8">
          ...
          <h1 className="font-display text-[clamp(48px,6vw,88px)] font-normal tracking-tight leading-[0.95] mb-6">
            От доски
            <br />
            до <em className="italic text-amber-deep font-medium">дома вашей мечты</em>
          </h1>
          <p className="text-lg text-ink-muted max-w-lg mb-8 leading-relaxed">
            Площадка, где встречаются заказчики, мастера-мебельщики и поставщики материалов
            Кыргызстана. Найдите идею, нарисуйте проект, получите предложение — всё в одном месте.
          </p>
        ...
        <div className="relative h-[460px] hidden lg:block">
          <div className="absolute top-0 right-10 w-[280px] -rotate-3 bg-paper rounded-2xl shadow-lift p-4 border border-line-soft">
            ...
            <div className="font-display text-[17px] font-semibold mb-1">Кухня «Алтын»</div>
            <div className="text-xs text-ink-muted">Усталык Studio • 14 дней</div>
          </div>
          <div className="absolute top-20 left-0 w-[240px] rotate-2 bg-wood-dark text-paper rounded-2xl shadow-lift p-4">
          ...
          <div className="absolute bottom-0 right-0 w-[260px] -rotate-1 bg-paper rounded-2xl shadow-lift p-4 border border-line-soft">
            <div className="h-[140px] rounded-lg mb-3 bg-gradient-to-br from-amber to-[#e89b4d]" />
            <div className="font-display text-[17px] font-semibold mb-1">Гардероб «Ала-Тоо»</div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-line border border-line rounded-2xl overflow-hidden mb-20">
        {[
          ...
          <div key={s.label} className="bg-paper p-7 transition-colors hover:bg-bg-warm">
            <div className="font-display text-4xl font-medium tracking-tight leading-none mb-1.5">
              {s.emphasis ? <em className="italic text-amber-deep">{s.num}</em> : s.num}
            </div>
      ...
      <section className="mb-20">
        <SectionHead
          ...
          action={
            <a href="#" className="text-sm font-semibold border-b border-current pb-0.5">
              Смотреть всё →
            </a>
          }
        />
        <div className="columns-2 md:columns-3 lg:columns-4 gap-5">
          {portfolio.map((item) => (
            ...
            >
              <div className={`mi-tag absolute top-3 left-3 z-10 bg-paper px-2.5 py-1 rounded-full text-[11px] font-bold`}>
                {categoryLabel(item.category)}
              </div>
              <div className={`${item.imageStyle} w-full relative overflow-hidden`} />
              <div className="absolute inset-0 bg-gradient-to-b from-transparent from-50% to-wood-dark/85 p-4 flex flex-col justify-end text-paper opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="text-xs opacity-80 mb-1">{item.authorHandle}</div>
                <div className="font-display text-base font-medium">{item.title}</div>
              </div>
            </Link>
          ))}
        </div>
