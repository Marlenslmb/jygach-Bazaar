...
const Constructor3DScene = lazy(() =>
  import("./constructor/Constructor3DScene").then((m) => ({
    default: m.Constructor3DScene,
  })),
);
...
export function ConstructorPage() {
  ...
  const extras =
    (state.hasLighting ? 8000 : 0) + (state.hasBackPanel ? 3500 : 0);
  ...
  const LeftPanel = () => (
    <div className="flex flex-col gap-3 h-full overflow-y-auto p-3 md:p-0">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {PRESETS.map((p) => (
            <button
              key={p.id}
              onClick={() => {
                dispatch({ type: "LOAD_PRESET", preset: p.id });
                showToast(`«${p.label}» загружен`);
                setMobileTab("canvas");
              }}
      ...
      <Panel title="Материал">
        <div className="grid grid-cols-3 gap-1.5 mb-2">
          {Object.values(MATERIALS).map((m) => (
            <button
              key={m.id}
              onClick={() =>
                dispatch({ type: "SET_MATERIAL", material: m.id as MaterialId })
              }
              title={m.label}
              className={cn(
                "aspect-square rounded-lg border-2 transition-all",
                state.material === m.id
                  ? "border-amber-deep scale-105 shadow-sm"
        ...
        <div className="text-center text-[11px] font-semibold text-ink-muted py-1 bg-bg-warm rounded-lg">
          {mat.label}
        </div>
      </Panel>
    </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  state.selectedId === el.id
                    ? "bg-amber-soft text-amber-deep font-bold"
                    : "hover:bg-bg-warm text-ink-soft",
                )}
              >
  ...
  const SettingsPanel = () => (
    <div className="flex flex-col gap-3 h-full overflow-y-auto p-3 md:p-0">
      {selectedEl && selectedEl.id !== "body" ? (
        <Panel title={`✦ ${selectedEl.label || selectedEl.type}`}>
          {[
            ...
            <PropRow key={label} label={label} value={val} onChange={onC} />
          ))}
        ...
        <Panel title="Параметры">
          ...
          <SwitchRow
            label="LED подсветка"
            hint={state.hasLighting ? "Включена" : "Выключена"}
            icon={<Lightbulb size={12} />}
            checked={state.hasLighting}
            onChange={() => dispatch({ type: "TOGGLE_LIGHTING" })}
            onColor="bg-amber"
          />
          <SwitchRow
            label="Задняя стенка"
            hint={state.hasBackPanel ? "Есть" : "Без стенки"}
            icon={<PanelLeft size={12} />}
            checked={state.hasBackPanel}
            onChange={() => dispatch({ type: "TOGGLE_BACK_PANEL" })}
            onColor="bg-moss"
          />
        </Panel>
      )}
      <Panel title="Смета">
        ...
        {state.hasLighting && (
          <CostLine label="Подсветка" value={8000} accent />
        )}
        {state.hasBackPanel && (
          <CostLine label="Задняя стенка" value={3500} accent />
        )}
        <div className="pt-2 mt-1 border-t border-line-soft flex justify-between items-center">
          ...
          <span className="font-display text-xl font-semibold text-amber-deep">
      ...
      <div className="flex items-center justify-between py-2 shrink-0 gap-2 flex-wrap">
        <div>
          <h1 className="font-display text-2xl md:text-3xl font-semibold tracking-tight leading-none">
            Конструктор{" "}
            <em className="italic text-amber-deep font-medium">мебели</em>
          </h1>
          <p className="text-[11px] text-ink-muted mt-0.5 hidden md:block">
            Drag — переместить · Углы — ресайз · ПКМ — меню · Del · Ctrl+Z ·
            Ctrl+D
          </p>
        </div>
        ...
        <div className="flex-1 flex flex-col bg-paper border border-line-soft rounded-2xl overflow-hidden min-w-0 shadow-soft">
          <div className="px-3 py-2 border-b border-line-soft bg-bg-warm/50 flex items-center gap-2 shrink-0">
            ...
            <div className="flex items-center gap-1 flex-1 justify-center">
              {(["totalWidth", "totalHeight", "totalDepth"] as const).map(
                (key, i) => (
                  <span key={key} className="flex items-center gap-1">
                    ...
                    <div className="flex items-center bg-paper border border-line rounded-md overflow-hidden">
                      <span className="text-[10px] font-bold text-ink-muted px-1.5 py-1 bg-bg-warm border-r border-line">
                        {i === 0 ? "Ш" : i === 1 ? "В" : "Г"}
                      </span>
                      <input
                        type="number"
        ...
        <div className="flex bg-bg-warm rounded-xl p-1 gap-1 shrink-0">
          {[
            ...
            <button
              ...
              className={cn(
                "flex-1 flex flex-col items-center gap-0.5 py-2 rounded-lg text-[10px] font-semibold transition-all",
                mobileTab === tab.id
...
function Panel({
  ...
  return (
    <div className="bg-paper border border-line-soft rounded-xl p-3 shrink-0">
      <div className="text-[10px] font-bold uppercase tracking-[0.13em] text-ink-muted mb-2">
        {title}
      </div>
      {children}
    </div>
...
function TBtn({
  ...
  return (
    <button
      ...
      title={title}
      className={cn(
        "w-7 h-7 rounded-md grid place-items-center text-ink-soft transition-colors",
        disabled
          ? "opacity-25 cursor-not-allowed"
          : "hover:bg-bg-warm hover:text-ink",
        className,
      )}
    >
      {children}
...
function PropRow({
  ...
  // Локальный строковый стейт — не сбрасываем фокус при вводе
  const [local, setLocal] = useState(String(value));

  // Синхронизируем если value изменился снаружи (drag, undo)
  // но только если поле не в фокусе
  ...
  const commit = () => {
    ...
    if (!isNaN(v) && v >= min) {
      ...
      // Откат к предыдущему значению без потери фокуса
      setLocal(String(value));
    }
  };
    ...
    <div className="flex justify-between items-center py-1.5 border-b border-line-soft/60 last:border-0">
      ...
      <input
        ...
        onKeyDown={(e) => {
          ...
          // Блокируем Delete/Backspace от всплытия — чтобы не удалял элемент
  ...
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-line-soft/60 last:border-0 gap-2">
      ...
      <button
        ...
        className={cn(
          "relative inline-flex shrink-0 rounded-full transition-colors duration-200 h-5 w-9",
          checked ? onColor : "bg-line",
        )}
      >
        <span
          className={cn(
            "pointer-events-none block rounded-full bg-white shadow-sm transition-transform duration-200 h-4 w-4 mt-0.5",
...
function CostLine({
  ...
  return (
    ...
    >
      <span className={accent ? "text-amber-deep" : "text-ink-muted"}>
        {label}
      </span>
