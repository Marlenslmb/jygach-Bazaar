...
import {
  User, Star, Package, MessageCircle, Heart,
  ClipboardList, Plus, Edit2, Trash2, LogOut,
  ChevronRight, CheckCircle2, Clock, TrendingUp,
  Briefcase, Camera, Phone, MapPin, Settings,
} from 'lucide-react'
...
function StatusBadge({ status }: { status: string }) {
  ...
  const s = map[status] ?? { label: status, class: 'bg-bg-warm text-ink-muted' }
  return (
    <span className={cn('text-[11px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider', s.class)}>
      {s.label}
    </span>
  )
}
...
function CustomerDashboard({ profile }: { profile: any }) {
  ...
  const stats = [
    { label: 'Мои заказы', value: myOrders.length, icon: <ClipboardList size={18} />, color: 'text-amber-deep' },
    { label: 'Избранных мастеров', value: favorites.masters.length, icon: <Heart size={18} />, color: 'text-moss' },
    { label: 'Активных откликов', value: myOrders.reduce((s: number, o: any) => s + o.bidsCount, 0), icon: <MessageCircle size={18} />, color: 'text-amber' },
  ]
    ...
    <div className="space-y-6">
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/orders/new" className="flex items-center gap-1.5 text-sm font-semibold text-amber-deep hover:text-amber-deep/80">
        ...
        {myOrders.length === 0 ? (
          <div className="text-center py-8 text-ink-muted">
            ...
            <p>Нет заказов. <Link to="/orders/new" className="text-amber-deep font-medium">Создать первый →</Link></p>
          </div>
        ) : (
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-display text-xl font-semibold">Избранные мастера</h3>
          <Link to="/masters" className="text-sm text-amber-deep font-semibold">Найти мастера →</Link>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link to="/constructor"
          ...
          <div className="text-xs text-ink-muted mt-1">Нарисовать и отправить мастеру</div>
        </Link>
        <Link to="/masters"
          className="card p-5 hover:border-amber text-center transition-all hover:-translate-y-0.5">
          <div className="text-2xl mb-2">👨‍🔧</div>
          <div className="font-semibold text-sm">Найти мастера</div>
          <div className="text-xs text-ink-muted mt-1">Каталог мастеров Кыргызстана</div>
        </Link>
      </div>
    </div>
  )
...
function MasterDashboard({ profile }: { profile: any }) {
  ...
  const [activeTab, setActiveTab] = useState<'orders' | 'portfolio' | 'services'>('orders')
  ...
  const stats = [
    { label: 'Рейтинг', value: '4.9', icon: <Star size={16} className="fill-amber text-amber" />, color: 'text-amber-deep' },
    { label: 'Проектов', value: '87', icon: <CheckCircle2 size={16} />, color: 'text-moss' },
    { label: 'Откликов', value: orders.length, icon: <ClipboardList size={16} />, color: 'text-amber' },
  ]
  ...
  return (
    <div className="space-y-6">
        ...
        {([
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={cn(
              ...
              activeTab === tab.id ? 'bg-paper shadow-sm text-ink' : 'text-ink-muted hover:text-ink'
            )}>
            {tab.label}
          </button>
        ))}
      ...
      {activeTab === 'orders' && (
        <div className="card p-6">
          <h3 className="font-display text-xl font-semibold mb-4">Доступные заказы</h3>
          <div className="space-y-3">
              ...
              <Link key={order.id} to={`/orders/${order.id}`}
                ...
                <div className="flex items-center gap-3 ml-3 shrink-0">
                  <div className="text-right">
                    ...
                    <div className="text-xs text-ink-muted">{order.deadlineWeeks} нед</div>
                  </div>
                  <ChevronRight size={16} className="text-ink-muted" />
                </div>
              </Link>
      ...
      {activeTab === 'portfolio' && (
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-display text-xl font-semibold">Моё портфолио</h3>
            <button onClick={() => showToast('Загрузка фото — скоро')}
      ...
      {activeTab === 'services' && (
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => showToast('Добавление услуги — скоро')}
              className="flex items-center gap-1.5 text-sm font-semibold text-amber-deep">
          ...
          <div className="space-y-3">
            {[
              { title: 'Кухонный гарнитур', price: 80000, desc: 'МДФ или массив' },
              { title: 'Шкаф-купе', price: 45000, desc: 'Любые размеры' },
              { title: 'Гардеробная', price: 120000, desc: 'Под ключ' },
            ].map((s, i) => (
              <div key={i} className="flex items-center justify-between p-4 bg-bg-warm rounded-xl">
                <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => showToast('Редактирование — скоро')}
...
function SupplierDashboard({ profile }: { profile: any }) {
  ...
  const stats = [
    { label: 'Материалов', value: materials.length, icon: <Package size={18} />, color: 'text-amber-deep' },
    { label: 'Просмотров', value: '1.2k', icon: <TrendingUp size={18} />, color: 'text-moss' },
    { label: 'Запросов', value: '24', icon: <MessageCircle size={18} />, color: 'text-amber' },
  ]
    ...
    <div className="space-y-6">
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => showToast('Добавление материала — скоро')}
            className="flex items-center gap-1.5 text-sm font-semibold text-amber-deep">
            <Plus size={15} /> Добавить
          </button>
        </div>
        ...
        {materials.length === 0 ? (
          <div className="text-center py-8 text-ink-muted">
            ...
            <button onClick={() => showToast('Форма добавления — скоро')}
              ...
              <div key={mat.id}
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <span className={cn('text-xs font-semibold px-2 py-1 rounded-full',
                    mat.in_stock ? 'bg-[#e6f0d8] text-[#4a5a2a]' : 'bg-bg-warm text-ink-muted')}>
      ...
      {/* Быстрые ссылки */}
      <div className="card p-6">
        <h3 className="font-display text-lg font-semibold mb-3">Каталог материалов</h3>
        <Link to="/materials" className="flex items-center justify-between p-3 bg-bg-warm rounded-xl hover:bg-line-soft transition-colors">
          <span className="text-sm font-medium">Посмотреть как видят покупатели</span>
          <ChevronRight size={16} className="text-ink-muted" />
        </Link>
      </div>
    </div>
...
function EditProfileModal({ profile, onClose }: { profile: any; onClose: () => void }) {
  ...
  const save = async () => {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) return
      await supabase.from('profiles').update({ name, phone, city, bio }).eq('id', user.id)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-wood-dark/40 backdrop-blur-sm">
      <div className="bg-paper rounded-2xl p-6 w-full max-w-md shadow-lift">
        <h3 className="font-display text-xl font-semibold mb-5">Редактировать профиль</h3>
        <div className="space-y-4">
          {[
            { label: 'Имя', value: name, set: setName, placeholder: 'Ваше имя' },
            { label: 'Телефон', value: phone, set: setPhone, placeholder: '+996 555 000 000' },
            { label: 'Город', value: city, set: setCity, placeholder: 'Бишкек' },
          ].map(f => (
            <div key={f.label}>
              <label className="block text-xs font-bold uppercase tracking-wider text-ink-muted mb-1.5">{f.label}</label>
              <input value={f.value} onChange={e => f.set(e.target.value)}
                placeholder={f.placeholder} className="form-input" />
            </div>
          ))}
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-ink-muted mb-1.5">О себе</label>
            <textarea value={bio} onChange={e => setBio(e.target.value)}
              rows={3} className="form-input resize-none" placeholder="Расскажите о себе..." />
          </div>
        </div>
        <div className="flex gap-3 mt-5">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-line font-semibold text-sm hover:bg-bg-warm">
            Отмена
          </button>
        ...
        <aside className="space-y-4">
          ...
          <div className="card p-6 text-center">
            ...
            <div className="relative inline-block mb-4">
              <div className="w-20 h-20 rounded-full bg-wood-dark text-amber-soft grid place-items-center font-display text-3xl font-bold mx-auto">
                {displayName[0]?.toUpperCase()}
              </div>
              <button onClick={() => showToast('Загрузка фото — скоро')}
                className="absolute bottom-0 right-0 w-7 h-7 bg-amber-deep text-paper rounded-full grid place-items-center border-2 border-paper hover:bg-amber transition-colors">
                <Camera size={12} />
              </button>
            </div>

            <h2 className="font-display text-xl font-semibold">{displayName}</h2>
            <div className="text-sm text-ink-muted mt-1">{profile?.email}</div>

            {/* Бейдж роли */}
            <div className={cn(
              'inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold mt-2',
              displayRole === 'master' ? 'bg-amber-soft text-amber-deep' :
              displayRole === 'supplier' ? 'bg-[#e6f0d8] text-[#4a5a2a]' :
              'bg-bg-warm text-ink-muted'
            )}>
              {displayRole === 'master' ? '👨‍🔧 Мастер' :
               displayRole === 'supplier' ? '📦 Поставщик' : '🛒 Заказчик'}
            </div>

            {profile?.city && (
              <div className="flex items-center justify-center gap-1 text-xs text-ink-muted mt-2">
          ...
          <div className="card p-3">
            {[
              ...
              { icon: <MessageCircle size={15} />, label: 'Сообщения', to: '/messages' },
              { icon: <Heart size={15} />, label: 'Избранное', to: '/masters' },
              { icon: <Settings size={15} />, label: 'Настройки', action: () => showToast('Настройки — скоро') },
            ].map((item, i) => (
              item.to ? (
                ...
                <button key={i} onClick={item.action ?? undefined}
                  className={cn(
                    ...
                    item.active ? 'bg-bg-warm text-ink font-semibold' : 'text-ink-soft hover:bg-bg-warm hover:text-ink'
                  )}>
                  {item.icon} {item.label}
                </button>
              )
              ...
              {(['customer', 'master', 'supplier'] as const).map(r => (
                <button key={r} onClick={() => setRole(r)}
                  className={cn(
                    ...
                    role === r ? 'bg-wood-dark text-paper' : 'bg-bg-warm text-ink-soft hover:bg-line-soft'
                  )}>
                  {r === 'customer' ? '🛒 Заказчик' : r === 'master' ? '👨‍🔧 Мастер' : '📦 Поставщик'}
                </button>
              ))}
            </div>
          </div>
        ...
        <main>
          ...
          <div className="mb-6">
            <h1 className="font-display text-4xl font-semibold tracking-tight">
              {role === 'master' ? 'Кабинет мастера' :
               role === 'supplier' ? 'Кабинет поставщика' : 'Личный кабинет'}
            </h1>
            <p className="text-ink-muted mt-1">
              {role === 'master' ? 'Управляйте заказами, портфолио и услугами' :
               role === 'supplier' ? 'Управляйте каталогом материалов' :
