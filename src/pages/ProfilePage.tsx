...
function MasterDashboard({ profile }: { profile: any }) {
  ...
  const { data: myPortfolio = [] } = useQuery({
    queryKey: ['my-portfolio', profile?.id],
    queryFn: () => mastersApi.getPortfolio(profile!.id),
    enabled: !!profile?.id,
  })
  ...
  if (isError || !profile) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        <p className="text-ink-muted">Не удалось загрузить профиль</p>
        <button onClick={() => navigate('/auth')}
          className="btn-primary">Войти заново</button>
      </div>
    )
  }
    ...
    <div className="animate-page-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-4 md:gap-6">
        ...
        <aside className="space-y-3 md:space-y-4">
          ...
          <div className="card p-4 md:p-6">
            ...
            <div className="flex items-center gap-4 md:flex-col md:items-center md:text-center">
              ...
              <div className="flex-1 min-w-0">
                <h2 className="font-display text-lg md:text-xl font-semibold truncate">{displayName}</h2>
                <div className="text-xs text-ink-muted mt-0.5 truncate">{profile?.email}</div>
                <div className={cn(
        ...
        <main>
          ...
          {role === 'master' && (
            <MasterDashboard profile={profile} />
          )}
